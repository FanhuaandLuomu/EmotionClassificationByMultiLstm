...
def load_data(my_model):
	...
	for i in range(360-len(em2)):
		em2.append(random.choice(em2[72:-30]))  # re-sampling 扩充到 450
...
def Multi_channel(models_all,X_train,X_test,Y_train,Y_test,test_label,X_val,Y_val):  # multi-channel 的神经网络
	...
	model.compile(loss='categorical_crossentropy',optimizer='adam')  #  配置网络 adam rmsprop 0.49206
	# 可以写个for循环 记录做大的acc 或 G-mean
	# X_val=[]
	# X_train2=[]
	# Y_val=[]
	...
	for i in range(nb_epoch):
		...
		hist=model.fit(X_train,Y_train[0], batch_size=32, nb_epoch=1, verbose=1,show_accuracy=True,   #20 10
