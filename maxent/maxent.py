...
def train(trainBinPath,modelPath):
	cmd='java -cp %s cc/mallet/classify/tui/Vectors2Classify --input %s --output-classifier \
		 %s --trainer MaxEnt' %(mallet_path,trainBinPath,modelPath)
	retcode=subprocess.Popen(cmd.split())
	retcode.wait()
	if(retcode<0):
		print('Train failed!')
...
def createPRF(FilePath,seed):  # 计算prf值 写入文件
	...
	f=open('resort_maxent_prf.txt','a')
...
def me_classify(trains,tests):
	...
	classify('train.model','test.bin','result.txt')
