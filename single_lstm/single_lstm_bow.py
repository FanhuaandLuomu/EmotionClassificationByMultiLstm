...
def createVocablulary(documents):  # 返回倒序排序的字典
	words={}
...
def createIndex(words,docs):  # 将文档矩阵转换为下标矩阵
	X_list=[]
	Y_list=[]
	...
	for i,document in enumerate(docs):
		...
		for word in document.words:
			if word in words:
				sen.append(words[word])
		# Y.append(document.polarity)
		# X.append(sen)
		X_list.append(sen)
		Y_list.append(document.polarity)
	return X_list,Y_list

def process(X, nb_words=max_features, skip_top=0,maxlen=None, start_char=1, oov_char=2, index_from=3):
...
# 	model.add(LSTM(lstm_output_dim,input_shape=(1,max_features)))  # LSTM 层 100->128
...
# 		hist=model.fit(X_train,Y_train, batch_size=32, nb_epoch=1, verbose=1, shuffle=True,  #20 10
...
def LSTM_model(X_train,X_test,Y_train,Y_test,test_label,X_val,Y_val):   # 生成LSTM网络
	...
	best_epoch=0
	best_weights_filepath='./best_weights.hdf5'
	saveBestModel=callbacks.ModelCheckpoint(best_weights_filepath,monitor='val_loss',verbose=3,
						save_best_only=True,mode='auto')
	earlyStopping=callbacks.EarlyStopping(monitor='val_acc',patience=0,verbose=1,mode='auto')
	hist=model.fit(X_train,Y_train,batch_size=32,nb_epoch=nb_epoch,verbose=1,shuffle=True,
				validation_data=(X_val,Y_val),callbacks=[earlyStopping])
	best_pred_label=model.predict_classes(X_test,batch_size=32,verbose=1)
	# acc=model.evaluate(X_test,Y_test,batch_size=32,verbose=1)
	# print acc
...
def run(seed):
	...
	X_train,X_test,Y_train,Y_test,X_val,Y_val,test_label=load_data(em0,em1,em2,em3,em4,em5,em6)
