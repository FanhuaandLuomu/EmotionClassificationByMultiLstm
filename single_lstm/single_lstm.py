...
def LSTM_model(X_train,X_test,Y_train,Y_test,test_label,X_val,Y_val):   # 生成LSTM网络
	...
	best_pred_prob=model.predict_proba(X_test,batch_size=32,verbose=1)
	best_pred_label=np.array([np.argsort(item)[-1] for item in best_pred_prob])
...
def run(seed):
	em0,em1,em2,em3,em4,em5,em6=Domain()
	print('len(em0):'+str(len(em0)))  # 669
	print('len(em1):'+str(len(em1)))   # 1392
	print('len(em2):'+str(len(em2)))   # 148
	print('len(em3):'+str(len(em3)))   # 1459
	print('len(em4):'+str(len(em4)))   # 2204
	print('len(em5):'+str(len(em5)))   # 1173
	print('len(em6):'+str(len(em6)))   # 362
